...
rewrites each deck on the fly before Chrome sees it:
...
hidden by Quarto's own html.print-pdf rules in print view, and annotate.js's
buttons and ink layer by the matching rules in annotate.scss — which also
means slide annotations don't reach these PDFs.)
...
Usage (manual):      python scripts/print_slides_pdfs.py [template.html]
"""
...
OUTPUT_DIR = "."
# This repo renders one deck at the project root. The course projects this
# script is shared with set these to "docs" / ".slides.html" and let the
# glob below find every deck.
DECK_SUFFIX = ".html"
DEFAULT_DECKS = ["template.html"]
...
def _collect_slides():
    """
    Decide which decks to convert, in priority order:

      1. CLI arguments — file paths or globs. Both deck and .pdf paths are
         accepted (a .pdf is mapped back to its deck source), and
      ...
      3. Neither — default to DEFAULT_DECKS.
    """
    if len(sys.argv) > 1:
        candidates = []
        ...
        if env.strip():
            ...
            candidates = DEFAULT_DECKS

    slides = []
    for path in candidates:
        path = re.sub(r"\.pdf$", DECK_SUFFIX, path.strip())
        if path.endswith(DECK_SUFFIX) and os.path.isfile(path) and path not in slides:
            slides.append(path)
    return slides
...
def main():
    ...
    if not slides_files:
        print("print_slides_pdfs: no decks to convert.")
        return
    ...
    def convert(file):
        pdf_file = re.sub(re.escape(DECK_SUFFIX) + r"$", ".pdf", file)
